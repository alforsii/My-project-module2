    ...
    function updateDeleteBtn() {
      ...
      sendBtns.forEach(sendBtn => {
        sendBtn.addEventListener('click', event => {
          event.preventDefault();
          const otherUserId = sendBtn.getAttribute('user_id');
          const otherUsername = sendBtn.getAttribute('_username');
          const otherFullName = sendBtn.getAttribute('_name');
          console.log('otherUserId: ', otherUserId);
          // socket.emit('receive-call-friends');
          socket.emit('call-socket-io', {
            id: otherUserId,
